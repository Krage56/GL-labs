...
void drawCube(GLfloat size, bool uncurtain = false)
{
    ...
    // правая грань
        glVertexPointer(3,
                        ...
                        0,
                        &right);
        glColor4f(1.0f, 0.5f, 0.0f, transparency);
        glDrawElements(GL_QUADS, 4, GL_UNSIGNED_BYTE, &ind);
...
void checkButton (GLFWwindow* window, int key, int scancode, int action, int mods) {
    static GLfloat transparency_step = -0.5f;
...
int main(void)
{
    ...
    window = glfwCreateWindow(720, 640, "Лабораторная №5", nullptr, nullptr);
    if (window == nullptr)
    {
